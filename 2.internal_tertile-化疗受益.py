...
def plot_tertile_km(
    ...
    for g in groups:
        ...
        ax.step(sf.index, sf[labels[g]], where='post', linewidth=3, color=colors[g], label=labels[g])
        ax.fill_between(
...
paths = glob.glob(f'results/moe_wsi/{RESULT_DIR}/summary_SXCH-Train_0.*.csv') + \
        glob.glob(f'results/moe_wsi/{RESULT_DIR}/summary_SXCH-Val_0.*.csv')
...
for path in paths:
    ...
    # ================= ALL KM (Tertile) =================
    cutoff_all = cutoff_df.loc[cutoff_df['group'] == 'ALL']
    cutoff1 = cutoff_all['cutoff1'].values[0]
    cutoff2 = cutoff_all['cutoff2'].values[0]

    # 划分三个风险组：0=低风险, 1=中风险, 2=高风险
    df['risk_group'] = 0  # 默认低风险
    df.loc[df['risk'] > cutoff1, 'risk_group'] = 1  # 中风险
    df.loc[df['risk'] > cutoff2, 'risk_group'] = 2  # 高风险
    print(df['risk_group'].value_counts())
    

    for risk_group in df['risk_group'].unique():
        df_risk = df[df['risk_group'] == risk_group].copy()
        plot_internal_km(
                data_df=df_risk,
                labels=['No Chemo', 'Chemo'],
                plot_type='Chemotherapy',
                save_path=f'{SAVE_DIR}/{RESULT_DIR}_tertile/Risk{risk_group}Chemo(ALL)_{center_name}.svg',
                fontsiz=FONTSIZE,
    ...
    for _, row in subgroup_cutoffs.iterrows():
        ...
        # 划分三个风险组：0=低风险, 1=中风险, 2=高风险
        df_sub['risk_group'] = 0  # 默认低风险
        df_sub.loc[df_sub['risk'] > cutoff1, 'risk_group'] = 1  # 中风险
        df_sub.loc[df_sub['risk'] > cutoff2, 'risk_group'] = 2  # 高风险
        ...
        for risk_group in df_sub['risk_group'].unique():
            ...
            plot_internal_km(
                    ...
                    labels=['No Chemo', 'Chemo'],
                    plot_type='Chemotherapy',
                    save_path=f'{SAVE_DIR}/{RESULT_DIR}_tertile/Risk{risk_group}Chemo({group_name})_{center_name}.svg',
                    fontsiz=FONTSIZE,
                    cutoff_time=CUTOFF_TIME,
                    survival_state=survival_state
                )