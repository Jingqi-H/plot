...
save_root = f'PLOTS/4.联合建模/ext'

external_path = glob.glob(f'PLOTS/4.联合建模/ext/*_result.xlsx')
for excel_path in external_path:
    center = os.path.basename(excel_path).split('_result.xlsx')[0]
    save_csv = f'{save_root}/{center}_cindex.csv'

    if 'SXCH' in excel_path:
        continue
    print(f'处理{center}')
