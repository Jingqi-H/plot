...
result_dir = "0.01loadloss"
moe_reatul_paht = f'results/moe_wsi/{result_dir}'
paths = glob.glob(f'{moe_reatul_paht}/summary_SXCH-*.csv') + glob.glob(f'{moe_reatul_paht}/results_external/summary_*.csv')
...
# reatul_paht = f'results/amil_wsi/patient_level_RandomTrainData'
# paths = glob.glob(f'{reatul_paht}/summary_SXCH-*.csv') + glob.glob(f'{reatul_paht}/results_external/summary_*.csv')
# print(f'Number of paths: {len(paths)}')
