...
def draw_scale_bar_on_roi(
    ...
    """
    在 PIL.Image ROI 图上绘制比例尺

    Parameters
    ----------
    roi_img : PIL.Image
    x_start, x_end : 原图中已知比例尺坐标
    scale_bar_mm : 原图线代表多少 mm（默认2mm）
    target_mm : ROI图想画多少 mm
    """

    if target_mm is None:
        target_mm = scale_bar_mm

    # 原图比例尺像素长度
    pixel_len_ref = abs(x_end - x_start)

    # 每毫米像素数
    pixel_per_mm = pixel_len_ref / scale_bar_mm

    # 新比例尺长度
    bar_len = int(pixel_per_mm * target_mm)
    ...
    # draw.text((x1, y - font_size - 8), text, fill=color, font=font)
...
def batch_extract_and_visualize_rois(
    ...
    *,
    ...
    # text = f"2 mm"
    # draw.text((x_start, bar_y - font_size - 8), text, fill="black", font=font)
    ...
    for i, center in enumerate(centers):
        ...
        # 在框的左上角绘制编号
        draw.text((x0_vis, y0_vis), str(i+1), fill=color)
...
def get_centers(coord_dir, file_name):
    ...
    centers1 = [i[7].tolist() for i in sorted_coords_list]
    return centers0, centers1


def drop_roi_in_img(meta, blend_img, downsample, expert_id, save_dir, drop_scale_bar=False,mpp=None):
...
for i, expert_id in enumerate([0,2,4,5]):
    ...
    drop_scale_bar = True if i == 0 else False
    drop_roi_in_img(meta, blend_img, down_sample, expert_id, save_dir, drop_scale_bar,mpp=mpp)
    # break
