...
for center in ['SXCH-Train+SXCH-Val','SXCH-Train', 'SXCH-Val', 'External', 'External(merged)']: # , 'YYH', 'SYSUCC'
    ...
    for sub_group in ['All', 'Stage_2', 'Stage_3']:
        ...
        for risk_group in df_cp['risk_group'].unique():
            ...
            if center == 'SXCH-Train+SXCH-Val':
                psm_df1 = pd.read_excel(psm_path.replace(center, 'SXCH-Train'), sheet_name=f'{sub_group}#{risk_group}', dtype={'Treated_CaseID': str, 'Control_CaseID': str})
                psm_df2 = pd.read_excel(psm_path.replace(center, 'SXCH-Val'), sheet_name=f'{sub_group}#{risk_group}', dtype={'Treated_CaseID': str, 'Control_CaseID': str})
                psm_df = pd.concat([psm_df1, psm_df2], axis=0)
            else:
                psm_df = pd.read_excel(psm_path, sheet_name=f'{sub_group}#{risk_group}', dtype={'Treated_CaseID': str, 'Control_CaseID': str})
            km_df = filter_psm_df(psm_df,df_risk)
            print(f'==================处理{center} ({sub_group}) (risk{risk_group})\nPSM前后样本数: {len(df_risk)} --{len(km_df)}')
