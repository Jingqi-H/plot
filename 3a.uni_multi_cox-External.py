...
def impute_missing_values(
    ...
    df_copy['LaurenType'] = df_copy['LaurenType'].map({0:1,1:1,2:2,3:3})

    return df_copy



'TCGA=STAD没有CEA、CA199、Lauren信息，这里的代码会插补'
...
# result_df = result_df[result_df['center'] != 'TCGA_STAD']  # 不用TCGA的数据

result_df = result_df[['case_id', 'risk', 'Age', 'Gender', 'CEA', 'CA199', 'Location', 'Grade', 'Histo Type',
...
'单因素分析'
remove_cols = ['case_id', 'slide_id', 'slide_id_wax', 'survival_time', 'event_times', 'censorship', 'status', 'DFS', 'DFS_status', 'OS', 'OS_status','center']
...
save_univariate_cox_results(
    uni_results_df,
    save_path=f'PLOTS/3.uni_multi_cox/{center}_uni_cox.xlsx',
...
covariates = ['risk', 'CEA', 'CA199', 'Gender_1', 'Chemotherapy_1',
              'pM', 'Age_1', 'pT_2', 'pT_3', 'pT_4', 'pN_1', 'pN_2', 'pN_3', #'Stage_2', 'Stage_3', 'Stage_4',
              'Grade_2', 'Location_2', 'Location_3', 'Location_4' #, 'LaurenType_2', 'LaurenType_3', 
              ]
cph = run_multivariate_cox(final_df, covariates)

save_multivariate_cox_results(
    cph.summary,
    save_path=f'PLOTS/3.uni_multi_cox/{center}_multi_cox.xlsx',
    rename_dict=rename_dict
)
...
plot_cox_forest_table(
    cph,
    rename_dict=rename_dict,
    save_path=f'PLOTS/3.uni_multi_cox/{center}_multi_plot.svg',
    fontsize=14
)
'合并单因素和多因素分析结果'
merge_uni_multi_cox(
    uni_path=f'PLOTS/3.uni_multi_cox/{center}_uni_cox.xlsx',
    multi_path=f'PLOTS/3.uni_multi_cox/{center}_multi_cox.xlsx',
    save_path=f'PLOTS/3.uni_multi_cox/{center}_uni&multi_cox.xlsx'
)
