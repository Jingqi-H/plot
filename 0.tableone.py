...
df.drop(columns=['case_id', 'OS', 'DFS'], inplace=True)
df['Age'] = np.where(
...
def calc_median_iqr_table(df, cohort_order):
    ...
    for cohort in cohort_order:
        ...
        result[cohort] = [os_stat, dfs_stat]
