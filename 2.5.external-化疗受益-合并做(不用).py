...
for center_name in centers:
    ...
    agg_df['center_name'] = center_name

    all_df = pd.concat([all_df, agg_df], axis=0)
    all_info_df = pd.concat([all_info_df, info_df], axis=0)

# 取group为ALL的cutoff_time
cutoff_time = cutoff_df[cutoff_df['group'] == 'ALL']['cutoff'].values[0]
all_df['risk_group'] = all_df['risk'].apply(lambda x: 0 if x <= cutoff_time else 1)
all_df.to_csv('test.csv', index=False)
all_info_df.to_csv('test2.csv', index=False)
...
for risk_group in all_df['risk_group'].unique():
    df_risk = all_df[all_df['risk_group'] == risk_group].copy()
    plot_internal_km(
            data_df=df_risk,
            labels=['No Chemo', 'Chemo'],
            plot_type='Chemotherapy',
            save_path=f'{SAVE_DIR}/Risk{risk_group}Chemo(ALL)_allcenter.svg',
            fontsiz=FONTSIZE,
...
for _, row in subgroup_cutoffs.iterrows():
    ...
    #转成整数
    all_info_df[subgroup_col] = all_info_df[subgroup_col].apply(lambda x: round(x) if pd.notna(x) else x)
    all_info_df[subgroup_col] = all_info_df[subgroup_col].astype('Int64')
    subgroup_cases = all_info_df.loc[
        all_info_df[subgroup_col] == int(subgroup_value),
        'case_id'
    ]

    df_sub = all_df[all_df['case_id'].isin(subgroup_cases)].copy()
    ...
    for risk_group in df_sub['risk_group'].unique():
        ...
        plot_internal_km(
                ...
                labels=['No Chemo', 'Chemo'],
                plot_type='Chemotherapy',
                save_path=f'{SAVE_DIR}/Risk{risk_group}Chemo({group_name})_allcenter.svg',
                fontsiz=FONTSIZE,
                cutoff_time=CUTOFF_TIME,
                survival_state=survival_state,
                center_name='External'
