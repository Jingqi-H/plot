...
def plot_internal_km(
    ...
    for g in groups:
        ...
        ax.step(sf.index, sf[labels[g]], where='post', linewidth=3, color=colors[g]) # where='post'表示事件发生后才下降
...
SAVE_DIR = f'PLOTS/2.km/{RESULT_DIR}/dfs'
...
paths = glob.glob(f'results/moe_wsi/{RESULT_DIR}/summary_SXCH-Train_dfs_slide*.csv') + \
        glob.glob(f'results/moe_wsi/{RESULT_DIR}/summary_SXCH-Val_dfs_slide*.csv')
...
for path in paths:
    ...
    plot_internal_km(
        ...
        save_path=f'{SAVE_DIR}/ALL_{center_name}.svg',
        fontsiz=FONTSIZE,
    ...
    for _, row in subgroup_cutoffs.iterrows():
        ...
        # if group_name != 'Pathological T stage_2':
        #     continue
